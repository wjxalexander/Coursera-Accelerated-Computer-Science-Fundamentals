...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  std::queue<TreeNode*> traversalQueue;  // use a queue first in first out
  auto rootNodePtr = tree.getRootPtr();
  if (!rootNodePtr) return results;
  traversalQueue.push(rootNodePtr);
  while (!traversalQueue.empty()) {
    auto frontTreeNodePtr = traversalQueue.front();
    for (auto childPtr : frontTreeNodePtr->childrenPtrs) {
      traversalQueue.push(childPtr);
    }
    results.push_back(frontTreeNodePtr->data);
    traversalQueue.pop();
  }
  return results;
}
