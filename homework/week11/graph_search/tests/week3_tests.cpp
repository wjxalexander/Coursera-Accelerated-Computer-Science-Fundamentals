...
// Autograder based on Zephyr test runner by Prof. Wade Fagen-Ulmschneider and
// the CS 225 Course Staff Based on Catch2 unit testing framework
...
TEST_CASE("Testing countEdges:", "[weight=1][ex1]") {
  ...
  SECTION("Should count 7 edges") { REQUIRE(numEdges == 7); }
}
  ...
  SECTION("Checking that point (2,2) was removed") {
    ...
    for (const auto& kv : graph.adjacencyMap) {
      ...
      if (k == IntPair(2, 2)) {
        ...
        // the value type is a set of neighboring points that are adjacent to
...
TEST_CASE("Testing graphBFS when the goal is unreachable:",
          "[weight=1][ex2][timeout=60000]") {
  GridGraph graph;
  ...
  std::cout
      ...
      << "This may take several seconds and display a warning (which is OK)."
      << std::endl
      << "If you want to cancel the test, press this key combination: Ctrl c"
      << std::endl
      << std::endl;
...
TEST_CASE("Testing graphBFS when start==goal:", "[weight=1][ex2]") {
  ...
  // somewhat redundant
  SECTION("Should return path of size 1") { REQUIRE(path.size() == 1); }
}
...
TEST_CASE("Testing graphBFS when a path can be found:", "[weight=1][ex2]") {
  ...
  SECTION("Path should not be empty") { REQUIRE(!path.empty()); };
...
TEST_CASE(
    "Testing puzzleBFS when the goal is unreachable (puzzle can't be solved):",
    "[weight=1][ex3][timeout=60000]") {
  ...
  SECTION("Path should be empty") { REQUIRE(path.empty()); }
}
