...
Node *_getMinNode(Node *left, Node *right) {
  if (!left) {
    return right;
  }
  if (!right) {
...
void _singleHeapDown(Node *n) {
  if (!n || (!n->left && !n->right)) {
    return;
  }
  const int currentValue = n->value;

  Node *minNode = _getMinNode(n->left, n->right);
  if (minNode->value < currentValue) {
    _swap(n, minNode);
    _singleHeapDown(minNode);
  }
}
